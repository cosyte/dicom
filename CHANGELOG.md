...
- bb9a7f0: **This is 0.1.0, the first release whose public API we treat as settled.**
...
- 47604f6: `deidentify()` now writes `(0012,0064)` De-identification Method Code Sequence on every run, beside
...
- 755e408: A DICOMDIR's Directory Record tree is now read, and `serializeDicom()` writes every record offset as
...
- 401909f: The data dictionary and the de-identification action table are now generated from the **PS3.6 and
...
- 6473903: `serializeDicom()` now writes objects under every Transfer Syntax PS3.5 2026c section A.4 names
...
- 0d21249: `parseDicom()` now reads objects under every Transfer Syntax PS3.5 2026c section A.4 names for
...
- c99016b: Docs: the README's first usage example and the quickstart's first example are now executed by the test suite, read straight out of the page they are printed on.
...
- 185c0ed: `PN`, `LO`, `SH`, `UC`, `ST`, `LT` and `UT` values now decode ISO 2022 code extensions when
...
- 8f35150: Read the file's own safe-private declaration, so a caller with a DICOM file from a vendor they hold
...
- 0bc40ae: Correct the troubleshooting remedy for `DICOM_DEIDENT_PRIVATE_CARRIER_NOT_AUDITABLE`: a value the file's own `(0008,0300)` declaration names safe is retained, not removed.
...
- 339892a: Add `RetainLongitudinalTemporalModifiedDates`, the second PS3.15 §E.3.6 Retain Longitudinal Temporal
...
- 9de3bf8: `serializeDicom()` now writes every Data Set in ascending tag order, so a file this library writes
...
- 94e28cc: `parseDicom()` now reads objects under the four JPIP Referenced Transfer Syntaxes of PS3.5 2026c:
...
- fad31fb: **`deidentify()` now removes a non-private attribute that neither this build's PS3.6 2026d data
