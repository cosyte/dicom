...
- ea6810a: Remove a retained private value this run did not enumerate, instead of shipping it under
  `(0012,0062) = YES`.
...
- ea6810a: **This is 0.1.0, the first release whose public API we treat as settled.**
...
- ea6810a: Add `toObject`, `toISO` and `toDate`, one conversion surface over the decoded `DA`, `TM` and `DT`
...
- ea6810a: Stop the de-identified header naming the sending site: replace the File Meta group and remove group 0004.
...
- ea6810a: Disclose a retained private value this run kept without enumerating it, instead of stamping
...
- ea6810a: Write `(0028,0303) Longitudinal Temporal Information Modified` on every `deidentify()` run, so a
...
- ea6810a: `deidentify()` now writes `(0012,0064)` De-identification Method Code Sequence on every run, beside
...
- ea6810a: A DICOMDIR's Directory Record tree is now read, and `serializeDicom()` writes every record offset as
...
- ea6810a: The data dictionary and the de-identification action table are now generated from the **PS3.6 and
...
- ea6810a: `serializeDicom()` now writes objects under every Transfer Syntax PS3.5 2026c section A.4 names
...
- ea6810a: `parseDicom()` now reads objects under every encapsulated Pixel Data Transfer Syntax that PS3.5
...
- ea6810a: Docs: the README's first usage example and the quickstart's first example are now executed by the test suite, read straight out of the page they are printed on.

  The README example claims its values as checked results now, against a synthetic CT object saved without its preamble, and one claim moved in the process: the study instance UID it shows is the one that object actually carries, not a placeholder. The quickstart's object is byte-identical to a fixture the test corpus builds, so the PHI scan reads it with the rest of the fixtures. Both examples are also compiled with the settings `tsc --init` writes for a new TypeScript project, so an example that does not compile fails the suite too. A changed value in either example fails the suite, and every install command the README and the installation page print is checked against the package's own name.

- ea6810a: Model CommonMark's HTML blocks in the PHI gate's override log, so a comment cannot exempt a scan
  target.
...
- ea6810a: `PN`, `LO`, `SH`, `UC`, `ST`, `LT` and `UT` values now decode ISO 2022 code extensions when
...
- ea6810a: Read the file's own safe-private declaration, so a caller with a DICOM file from a vendor they hold
...
- ea6810a: Gate the public surface against internal project bookkeeping, and sweep the tree it would have failed.
...
- ea6810a: Correct the troubleshooting remedy for `DICOM_DEIDENT_PRIVATE_CARRIER_NOT_AUDITABLE`: a value the file's own `(0008,0300)` declaration names safe is retained, not removed.
...
- ea6810a: Add `RetainLongitudinalTemporalModifiedDates`, the second PS3.15 §E.3.6 Retain Longitudinal Temporal
...
- ea6810a: Claim the PHI gate's `CRLF` line split, and delete the disclosure that said it could not be claimed.
...
- ea6810a: Refuse a `###` run an invisible character separates from the path, in the PHI gate's override log.
...
- ea6810a: Withhold the PHI gate's verdict over a target it enumerated and never opened, and put the install
...
- ea6810a: `serializeDicom()` now writes every Data Set in ascending tag order, so a file this library writes
...
- ea6810a: Parse the PHI gate's override log with CommonMark's line ending, and pin the spec that says so.
...
- ea6810a: `parseDicom()` now reads objects under the four JPIP Referenced Transfer Syntaxes of PS3.5 2026c:
...
- ea6810a: **`deidentify()` now removes a non-private attribute that neither this build's PS3.6 2026d data
