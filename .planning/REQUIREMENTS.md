...
- [x] **TOL-07** — Odd-length values are tolerated (padded per VR rules — space for text VRs, NULL for UI, NULL for OB) and emit `DICOM_ODD_LENGTH_VALUE_PADDED`; strict mode throws.
- [x] **TOL-08** — A known tag whose in-file VR disagrees with the dictionary VR is tolerated: the parser trusts the encoded VR, records the deviation as `DICOM_VR_MISMATCH`, and keeps parsing; strict mode throws.
