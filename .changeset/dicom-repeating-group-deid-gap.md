...
Fix a de-identification defect: `deidentify()` never removed and never reported the attributes
PS3.15 marks for removal as a repeating-group family, so a file carrying `(6000,4000)` Overlay
Comments came back with a clean report and the text still in it.
