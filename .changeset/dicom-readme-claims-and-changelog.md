...
`assets/published-urls.json`, which that file's own `$comment` describes as a declaration made from
...
- **Element access.** `Dataset.get` / `has` / `getAll` take the 8-character tag (`"00080060"`,
  ...
  `Dictionary.byKeyword(...)?.tag` shown for resolving a keyword first.
- **Transfer syntaxes.** The README said the supported set was the four v1 syntaxes "and any
  ...
  synthetic object in `1.2.840.10008.1.2.4.50` and catching the throw. The "index a folder of
  studies" recipe promised that nothing there throws on a quirky file; it now says to catch
  `DicomParseError` per file and skip.
