...
The converse does not hold: a conformant 40-character Value under an `(0008,0005)` of `ISO_IR 192`
is 120 bytes and raises the code, which a test pins with a file that really does declare the
repertoire rather than a fixture that asserts bytes are characters. Trailing pad counts too, and
errs the same way. Read the code as "a Value in this attribute is over 64 bytes", which is what is
measured, never as "the attribute is non-conformant".
...
when they are equal. `position.byteOffset` locates the element. Emitted by `deidentify()` only, so
it never reaches the parser's `{ strict: true }` escalation and cannot refuse a conformant file.
