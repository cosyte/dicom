...
2.1; the allow list keeps `/\r?\n/` deliberately, because unifying them would widen the allow list
and a live allow entry suppresses a hit. No direction is claimed for either split: on one measured
log the two entry sets are disjoint. The spec is vendored under `vendor/commonmark/` and re-hashed
