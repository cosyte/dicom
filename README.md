...
- **Spec-clean serializer**: `serializeDicom(ds)` round-trips a dataset back to Part 10 bytes in its source transfer syntax (no transcode; the four native syntaxes only, so an encapsulated object is refused), with correct File Meta group length, even-length padding, every Data Set it can walk (the root and each Sequence Item, at every depth up to `NESTING_DEPTH_LIMIT`) emitted in ascending tag order with Items kept in order and no value changed, and lossless File Meta: non-modeled `(0002,xxxx)` elements are preserved and re-emitted in tag order. Not ordered: a tag repeated inside an Item (both copies kept), a Sequence that cannot be walked, is nested past the bound or whose parsed `items` do not match its bytes, and any `UN`-carried Sequence. Written last whatever its tag: an element whose own bytes do not show where a reader ends it, such as an undefined-length `UN` the parser could not read as a Sequence.
...
The buffers are views over the parsed element unless you parse with `{ copyValues: true }`. **The limits sit beside it**: the pixels are never decoded, and `serializeDicom` refuses every section A.4 syntax, so a compressed object read here, de-identified or not, is not written back out by this package.
...
// (throws UNSUPPORTED_TRANSFER_SYNTAX for a PS3.5 2026c section A.4 compressed object: this
// package de-identifies its metadata but does not write an encapsulated syntax)
