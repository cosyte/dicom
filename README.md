...
- **A sequence the parser could not open.** `deidentify()` recurses only into a sequence whose items the parser materialized, so anything nested inside one it could not read is neither cleaned nor listed in the report. Two shapes reach that state, and **both announce themselves**: an undefined-length `UN` value the CP-246 descent could not read as a sequence, and a defined-length Implicit VR LE value whose dictionary-resolved `SQ` is not an item stream (`DICOM_SQ_NOT_DESCENDED`). Read `ds.warnings` before treating a `DeidentifyReport` as complete.
