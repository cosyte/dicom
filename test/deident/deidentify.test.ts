...
describe("deidentify - private attributes", () => {
  ...
  describe("RetainSafePrivate scopes block reservations per Data Set (PS3.5 §7.8.1)", () => {
    ...
    it("removes an item's private element whose reservation exists only at the root", () => {
      // The third direction, and a real behaviour change: a sender that declares
      // the creator once at the root and puts private data in items (Per-Frame
      // Functional Groups do this) loses those elements. PS3.5 §7.8.1 scopes the
      // reservation to the Data Set the creator appears in, so the item has no
      // reservation at all and nothing vouches for the element. Fail-safe, and
      // the conformant reading, but it is a loss and is recorded as one rather
      // than discovered.
      const ds = parseDicom(
        buildDicom({
          transferSyntax: TS_EXPLICIT_LE,
          elements: [
            ...
            {
              tag: "00081115",
              items: [{ elements: [{ tag: "00091001", vr: "LO", value: pad("acme-safe") }] }],
            },
          ],
        }),
      );
