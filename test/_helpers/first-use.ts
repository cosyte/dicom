...
/** The bytes of the first `Buffer.from("<base64>", "base64")` a snippet builds, or `undefined`. */
export function base64Object(code: string): Buffer | undefined {
  const found = /Buffer\.from\(\s*"([A-Za-z0-9+/=]+)",\s*"base64"/.exec(code);
  return found?.[1] === undefined ? undefined : Buffer.from(found[1], "base64");
}
